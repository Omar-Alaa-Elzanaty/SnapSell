...
public sealed class AccountController(ICacheService cacheService,ISender sender) : ApiControllerBase(cacheService)
{
    ...
    public async Task<IActionResult> RegisterSeller([FromForm] RegisterRequestSellerDto request, CancellationToken cancellationToken)
    {
    ...
    public async Task<IActionResult> RegisterCustomer([FromForm] RegisterRequestCustomerDto request, CancellationToken cancellationToken)
    {
