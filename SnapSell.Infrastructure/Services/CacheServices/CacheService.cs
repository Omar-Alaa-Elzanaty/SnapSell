...
namespace SnapSell.Infrastructure.Services.CacheServices
{
    public class CacheService : ICacheService
    {
        ...
        public async Task<CacheResponse> GetCacheCodes()
        {
            ...
            {
                var categoryCache = await _unitOfWork.CacheCodesRepo.Entites.FirstAsync(x => x.CacheKey == typeof(CategoryCache).Name);
                var brandCache = await _unitOfWork.CacheCodesRepo.Entites.FirstAsync(x => x.CacheKey == typeof(BrandCache).Name);
            ...
            if (bool.TryParse(_contextAccessor.HttpContext?.Response.Headers["UserCache"].ToString(), out bool isUserOn) && isUserOn)
            {
                ...
                var profileCache = await _unitOfWork.CacheCodesRepo.Entites.FirstAsync(x => x.CacheKey == typeof(ProfileCache).Name && x.UserId == userId);
        ...
        {
            var generalProperty = await _unitOfWork.CacheCodesRepo.Entites.FirstAsync(x => x.CacheKey == typeof(T).Name);
        ...
        {
            var userId = _contextAccessor.HttpContext?.User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
            var userProperty = await _unitOfWork.CacheCodesRepo.Entites.FirstAsync(x => x.CacheKey == typeof(T).Name && x.UserId == userId);
