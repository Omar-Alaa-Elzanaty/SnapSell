...
{
    internal sealed class GetRelatedProductsQueryWithPaginationHandler
        : IRequestHandler<GetRelatedProductsQueryWithPagination,
            PaginatedResult<GetRelatedProductsQueryWithPaginationDto>>
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (product == null)
            {
                return await PaginatedResult<GetRelatedProductsQueryWithPaginationDto>
                    .FailureAsync(
            ...
            return await _unitOfWork.ProductsRepo.Entities
                .Where(x =>
                    (x.Categories.Any(c =>
                        product.Categories.Select(productCategory => productCategory.CategoryId)
                            .Contains(c.CategoryId)) && x.BrandId == product.BrandId && x.StoreId == product.StoreId)
                    || (x.Categories.Any(c =>
                        product.Categories.Select(productCategory => productCategory.CategoryId)
                            .Contains(c.CategoryId)) && x.StoreId == product.StoreId)
                    || (x.Categories.Any(c =>
                        product.Categories.Select(productCategory => productCategory.CategoryId)
                            .Contains(c.CategoryId)) && x.BrandId == product.BrandId)
                    || x.Categories.Any(c =>
                        product.Categories.Select(productCategory => productCategory.CategoryId)
